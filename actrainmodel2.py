...
class BertForSequenceClassificationMultiLabel(BertForSequenceClassification):
    def __init__(self, config):
        ...
        self.linear = torch.nn.Linear(config.hidden_size+1, config.num_labels)
    ...
    def forward(
        ...
        # 議員フラグテンソルを結合
        giin_flags_tensor=giin_flags_tensor
        #print(f"\n{giin_flags_tensor}\n{averaged_hidden_state}")
        with_giin_flag = torch.cat((averaged_hidden_state, giin_flags_tensor), 1)
        #print(with_giin_flag)
        # 線形変換
        # scores = self.linear(averaged_hidden_state) 
        scores = self.linear(with_giin_flag) 
        # 出力の形式を整える。
...
class BertForSequenceClassification_pl(pl.LightningModule):
    ...
    def __init__(self, model_name, num_labels, lr):
        ...
        # BERTのロード
        self.bert_sc = BertForSequenceClassificationMultiLabel.from_pretrained(
            model_name,
    ...
    def test_step(self, batch, batch_idx):
        ...
        output = self.bert_sc(**batch)
        a = output.logits.argmax(-1)
        labels_predicted= torch.nn.functional.one_hot(a, num_classes=len(ARGUMENT_CLASSES))
...
def BAM_aC(sentences,proc: MoneyExpression,giin):
    ...
    for procidx,mex in enumerate(proc):
        ...
        tmplabel = [0] * len(ARGUMENT_CLASSES)
        tmplabel[ARGUMENT_CLASSES.index(mex.argumentClass)] = 1
        encoding['labels'] = tmplabel
        encoding["giin_flags_tensor"] = [giin]
...
def estimate_local(minutesObj: MinutesObject):
    ...
    for proc_obj in tqdm(minutesObj.local):
        ...
        for procidx,proc in enumerate(proc_obj.proceeding):
            ...
            #argumentClass
            for d in BAM_aC(sentences,proc.moneyExpressions,giin):
                dmemo2.append(d)
...
def estimate_diet(minutesObj: MinutesObject):
    ...
    for proc_obj in tqdm(minutesObj.diet):
        ...
        for spidx,sp in enumerate(proc_obj.speechRecord):
            ...
            if sp.speakerPosition==None and sp.speech[:8].find("委員長")<0:
                giin = 1
            else:
                giin = 0

            #argumentClassを付ける
            for d in BAM_aC(sentences,sp.moneyExpressions,giin):
                dmemo2.append(d)
...
if __name__ == "__main__":
    ...
    model.bert_sc.save_pretrained('BAM/ac_model_transformers2')
    #torch.save(model.state_dict(), 'BAM/ac_model_transformers2')